...
**Vulkan compute preview model list lives in `PathTracingRenderer/project_settings.json`, not in code.** The "Vulkan Mode" model dropdown is populated from that manifest (`{"models":[{"name","folder"}, …]}`). There are **no hardcoded models** — `vulkan_compute_preview.cpp` reads the manifest once via `projectSettingsState()`/`loadProjectSettings()` (folders resolve relative to CWD and its parent, so repo-root `assets/<dir>` entries work when run from `PathTracingRenderer/`). The "Import Model Folder" / Browse / Paste UI calls `VulkanComputePreview::importModelFromFolder`, which finds the folder's `scene.gltf`/`.glb`, appends a `ModelEntry`, and **persists the updated list back to `project_settings.json`** so imports survive across runs; folders inside the app CWD or repo root are saved as portable relative paths, while external folders remain absolute. Successful scene selection writes `lastSelectedModelFolder`, and startup loads that model directly; clearing selection removes it and starts the Vulkan viewport on the default procedural scene (base, cube, sphere). Missing folders are logged and skipped, not fatal. Touching the model list from more than the main thread would need synchronization — today it is main-thread only.
...
**Vulkan lighting/performance debugging.** Selectable `.exr`/`.hdr` skies are sampled by the Vulkan glTF compute preview through descriptor binding 17, with a 1x1 fallback image kept bound when no environment is available. The 3-point key/fill/rim lights are direct analytic lights controlled by `VulkanPreviewSettings` and `GpuSettings`. Disabled lights must be cheap: if the sun is disabled or has zero intensity, `directSunLighting` should return before shadow visibility. Shadows are gated by a global master (`lightingDebug.pointLightShadows` / `GpuSettings.lightingControls.x`, default off) exposed as `Scene -> Vulkan -> Lighting Debug -> Enable Shadows (master)` or `NrayRenderDocHeadless --point-light-shadows`; the master gates ALL finite BVH shadows (sun + key/fill/rim). Each point light has its own gate (`keyLightShadows`/`fillLightShadows`/`rimLightShadows` in `GpuSettings.pointLightShadowFlags`, default on; effective shadow = master AND per-light), driven by the UI "Key/Fill/Rim Shadows" checkboxes or headless `--no-{key,fill,rim}-light-shadows`. Use `--lighting-log` or `NRAY_VULKAN_LIGHTING_LOG=1` plus small headless A/B renders to compare `gpuDispatchMs` before blaming denoising, HDRI sampling, or material code for lighting regressions.

**RenderDoc Vulkan debugging.** Use `docs/renderdoc-vulkan-debugging.md` for manual frame captures when Vulkan preview evidence needs GPU-resource inspection. Launch the executable from RenderDoc with an absolute path (repo-relative target `bin/Release/PathTracingRenderer` or `bin/Debug/PathTracingRenderer`) and set the working directory to the repo's `PathTracingRenderer/` folder so `models/`, `textures/`, and the Vulkan preview manifest resolve correctly. Treat RenderDoc as a frame/resource debugger for compute dispatches, descriptor bindings, SSBO contents, storage images, glTF textures, denoiser resources, and shadow/refraction debugging; it is not a replacement for compile/link validation. If capture does not attach, check Vulkan capture-layer registration before debugging N-Ray. RenderDoc can replay hardware ray-tracing pipeline results for later passes, but Vulkan/D3D12 ray-tracing pipeline work itself is opaque and cannot be introspected. For agent-run headless capture/report work, use `bin/Release/NrayRenderDocHeadless` (built by `make build-renderdoc-headless`): drives `VulkanComputePreview` directly with no raylib/ImGui, captures via `renderdoccmd`, emits JSON and PPM, and never writes `project_settings.json`. See `docs/renderdoc-vulkan-debugging.md#headless-agent-workflow` for verified commands, JSON schema, and known quirks. For docs-only debugging-guide work, avoid unnecessary builds because the raylib HDR setup can leave `vendor/raylib` dirty.

There are **no automated tests** — verification is manual (run the renderer, move the camera, use the debug ray / stats panel). For Vulkan preview throughput checks, prefer `bin/Release/NrayRenderDocHeadless` with fixed `--width`, `--height`, `--samples`, `--max-bounces`, model selection, and lighting flags, then compare the JSON `gpuDispatchMs` and `lighting` fields. See README.md for controls (WASD + RMB camera, LMB debug ray / model select / click-DoF).
